...
/**
 * Implements BirdersRepository interface.
 * Handles communication with MongoDB database
 * @author Beatrice
 * @since 1.0
 *
 */
public class BirdersRepoMongo implements BirdersRepository {
	...
	public User sync(User user) {
		System.out.println("In sync BirdersRepo, user: " + user.toString());
		...
		if(result == null) {
			...
			System.out.println("JsonObj from sync BirdersRepo" +jsonObj);
	...
	public User updateUserLists(User user) {
		System.out.println("Updating users list, birdersRepo. List: " + gson.toJson(user));
